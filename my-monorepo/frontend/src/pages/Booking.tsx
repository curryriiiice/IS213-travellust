  ...
  const handleConfirmBooking = () => {
    ...
    if (incomplete) {
      toast({
        ...
        description:
          'Please fill in at least name and email for all passengers.',
        variant: 'destructive',
      });
      return;
    }
    ...
    addBooking({
      ...
      itemType:
        state.itemType === 'node'
          ? state.data.type === 'hotel'
            ? 'hotel'
            : state.data.type === 'flight'
              ? 'flight'
              : 'attraction'
          : state.itemType,
      title: itemTitle,
        ...
        >
          ...
          <div className="bg-card border border-border rounded-sm p-5 mb-6">
            ...
            {state.itemType === 'flight' && (
              <p className="text-sm text-muted-foreground mt-0.5">
                {(state.data as FlightOffer).airline} ·{' '}
                {(state.data as FlightOffer).flightNumber} ·{' '}
                {(state.data as FlightOffer).departureTime} –{' '}
                {(state.data as FlightOffer).arrivalTime}
              </p>
            )}
            <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
              <span className="text-sm text-muted-foreground">
                ${unitPrice.toLocaleString()} × {passengers.length}{' '}
                {passengers.length > 1 ? 'tickets' : 'ticket'}
              </span>
              <span className="text-xl font-mono tabular-nums font-medium">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium">{guestLabel} Details</h2>
              <Button
                variant="outline"
                size="sm"
